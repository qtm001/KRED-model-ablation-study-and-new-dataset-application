...
class Trainer(BaseTrainer):
    ...
    def _valid_epoch(self, epoch):
        ...
        self.model.eval()
        y_pred = []
        start_list = list(range(0, int(len(self.test_data['label'])/20), int(self.config['data_loader']['batch_size'])))
        for start in tqdm(start_list):
            if start + int(self.config['data_loader']['batch_size']) <= int(len(self.test_data['label'])/20):
                end = start + int(self.config['data_loader']['batch_size'])
            else:
                end = int(len(self.test_data['label'])/20)
            out = self.model(self.test_data['item1'][start:end], self.test_data['item2'][start:end], self.config['trainer']['task'])[
                0].cpu().data.numpy()

            y_pred.extend(out)
        n_truth = int(len(self.test_data['label'])/20)
