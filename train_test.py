...
def testing(test_data, config):
    if config['trainer']['task'] == "user2item":
        ...
        task_index = 3
    model = torch.load('./out/saved/models/KRED/checkpoint_{}.pt'.format(config['trainer']['task']), map_location=lambda storage, loc: storage.cuda(0))
    print('Test, model loaded; starting evaluation, the task is {}'.format(config['trainer']['task']))
    model.eval()
    y_pred = []
    start_list = list(range(0, int(len(test_data['label'])*0.1), config['data_loader']['batch_size']))
    for start in tqdm(start_list):
        ...
        # out = model(test_data['user_id'][start:end], test_data['news_id'][start:end], config['data_loader']['batch_size'])[task_index].cpu().data.numpy()
        out = model(test_data['item1'][start:end], test_data['item2'][start:end], config['data_loader']['batch_size'])[task_index].cpu().data.numpy()

        
        y_pred.extend(out)
    truth = test_data['label'][:len(y_pred)]
