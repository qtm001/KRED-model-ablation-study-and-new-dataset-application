...
class News_embedding(nn.Module):
    ...
    def forward(self, news_id):
        ...
        # if self.config['model']['distillation'] == False:
        #     #print('distillation deactivated')
        #     concat_embedding = torch.cat([news_entity_embedding, torch.FloatTensor(np.array(context_vecs)).cuda()],
        #                             len(news_entity_embedding.shape) - 1)
        # else:
        #     #print('distillation activated')
