...
class News_embedding(nn.Module):
    ...
    def attention_layer(self, entity_embeddings, context_vecs):
        if self.config['model']['distillation'] == False:
            return torch.sum(entity_embeddings, dim=-2), 1
    ...
    def get_entities_ids(self, news_id):
        ...
        for news in news_id:
            if len(news) == 0:
                entities.append(self.doc_feature_dict['N0'][0])
            elif type(news) == str:
    ...
    def get_entities_nums(self, news_id):
        ...
        for news in news_id:
            if len(news) == 0:
                entities_nums.append(self.doc_feature_dict['N0'][1])
            elif type(news) == str:
    ...
    def get_position(self, news_id):
        ...
        for news in news_id:
            if len(news) == 0:
                istitles.append(self.doc_feature_dict['N0'][2])
            elif type(news) == str:
    ...
    def get_type(self, news_id):
        ...
        for news in news_id:
            if len(news) == 0:
                istopics.append(self.doc_feature_dict['N0'][3])
            elif type(news) == str:
    ...
    def get_context_vector(self, news_id):
        ...
        for news in news_id:
            if len(news) == 0:
                context_vectors.append(self.doc_feature_dict['N0'][4])
            elif type(news) == str:
    ...
    def forward(self, news_id):
        ...
        if self.config['model']['kgat'] == False: 
            ...
            if self.config['model']['context'] == False:
                ...
                news_entity_embedding = kgat_entity_embeddings
            else:
        ...
        concat_embedding = torch.cat([aggregate_embedding, torch.FloatTensor(np.array(context_vecs)).cuda()],
