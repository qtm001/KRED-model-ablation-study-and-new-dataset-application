...
class KREDModel(nn.Module):
    ...
    def forward(self, user_features, news_features, task):
        if self.config['trainer']['task'] == 'item2item':
            user_embedding, top_index = self.news_embedding(user_features)
            ...
            user_embedding, top_index = self.news_embedding(news_features)
            candidate_news_embedding, topk_index = self.news_embedding(news_features)
        else:
